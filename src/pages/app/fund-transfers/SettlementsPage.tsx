...
const SETTLEMENT_TYPES: FundTransferTransactionType[] = ["SETTLEMENT", "FX_FEE", "ADJUSTMENT"];
...
export default function SettlementsPage() {
  ...
  const [type, setType] = useState<FundTransferTransactionType>("SETTLEMENT");
  const [direction, setDirection] = useState<TransferDirection>(TYPE_DEFAULT_DIRECTION.SETTLEMENT);
  ...
  async function load() {
    ...
    try {
      ...
      setItems((res.items || []).filter((i) => SETTLEMENT_TYPES.includes(i.type)));
    } catch (e: any) {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      toast.success("Settlement transaction saved");
      setAmount("");
      setReference("");
      setNote("");
      await load();
    } catch (e: any) {
      toast.error(e?.message || "Failed to save settlement transaction");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">Record settlement, FX fee, and adjustment entries.</p>
      </div>

      <Card className="rounded-2xl border-indigo-100 bg-white shadow-sm">
        <CardHeader><CardTitle className="text-base">Record Settlement Transaction</CardTitle></CardHeader>
        <CardContent>
          <form onSubmit={submit} className="grid gap-3 md:grid-cols-3">
            <div className="space-y-1">
              ...
              <select className="h-10 w-full rounded-md border border-indigo-200 bg-white px-3 text-sm" value={type} onChange={(e) => onTypeChange(e.target.value as FundTransferTransactionType)}>
                {SETTLEMENT_TYPES.map((opt) => <option key={opt} value={opt}>{opt}</option>)}
              </select>
            </div>
            <div className="space-y-1">
      ...
      <Card className="rounded-2xl border-indigo-100 bg-white shadow-sm">
        <CardHeader><CardTitle className="text-base">Settlement Activity</CardTitle></CardHeader>
        <CardContent className="p-0">
          {loading ? (
            <div className="p-6 text-sm text-slate-500">Loading...</div>
          ) : items.length === 0 ? (
            <div className="p-6 text-sm text-slate-500">No settlement activity yet.</div>
          ) : (
